import {
  ...
  type TransferOrderLineItem,
  type InsertTransferOrderLineItem,
  serialNumbers,
  type SerialNumber,
  type InsertSerialNumber,
  batchNumbers,
  type BatchNumber,
  type InsertBatchNumber,
  stockAdjustments,
  type StockAdjustment,
  type InsertStockAdjustment,
  stockAdjustmentLineItems,
  type StockAdjustmentLineItem,
  type InsertStockAdjustmentLineItem,
  stockCounts,
  type StockCount,
  type InsertStockCount,
  stockCountLineItems,
  type StockCountLineItem,
  type InsertStockCountLineItem,
  compositeItemComponents,
  type CompositeItemComponent,
  type InsertCompositeItemComponent,
  uomConversions,
  type UOMConversion,
  type InsertUOMConversion,
  inventoryCostLayers,
...
export interface IStorage {
  ...
  getWarehouse(tenantId: string, id: string): Promise<Warehouse | null>;
  createWarehouse(data: InsertWarehouse & { tenantId: string }): Promise<Warehouse>;
  updateWarehouse(tenantId: string, id: string, data: Partial<InsertWarehouse>): Promise<Warehouse>;
  deleteWarehouse(tenantId: string, id: string): Promise<void>;
  getDefaultWarehouse(tenantId: string): Promise<Warehouse | null>;
  setDefaultWarehouse(tenantId: string, id: string): Promise<Warehouse>;

  // Inventory - Warehouse Stock
  getWarehouseStock(tenantId: string, filters?: { warehouseId?: string; itemId?: string }): Promise<WarehouseStock[]>;
  getWarehouseStockByItem(tenantId: string, itemId: string): Promise<WarehouseStock[]>;
  getWarehouseStockByWarehouse(tenantId: string, warehouseId: string): Promise<WarehouseStock[]>;
  upsertWarehouseStock(data: InsertWarehouseStock & { tenantId: string }): Promise<WarehouseStock>;
  adjustWarehouseStock(tenantId: string, warehouseId: string, itemId: string, quantityChange: number): Promise<WarehouseStock>;

  // Inventory - Transfer Orders
  getTransferOrders(tenantId: string, filters?: { status?: string; fromWarehouseId?: string; toWarehouseId?: string }): Promise<TransferOrder[]>;
  getTransferOrder(tenantId: string, id: string): Promise<TransferOrder | null>;
  createTransferOrder(data: InsertTransferOrder & { tenantId: string }, lineItems: InsertTransferOrderLineItem[]): Promise<TransferOrder>;
  updateTransferOrder(tenantId: string, id: string, data: Partial<InsertTransferOrder>): Promise<TransferOrder>;
  deleteTransferOrder(tenantId: string, id: string): Promise<void>;
  updateTransferOrderStatus(tenantId: string, id: string, status: string): Promise<TransferOrder>;
  getTransferOrderLineItems(tenantId: string, transferOrderId: string): Promise<TransferOrderLineItem[]>;

  // Inventory - Serial Numbers
  getSerialNumbers(tenantId: string, filters?: { itemId?: string; warehouseId?: string; status?: string }): Promise<SerialNumber[]>;
  getSerialNumber(tenantId: string, id: string): Promise<SerialNumber | null>;
  createSerialNumber(data: InsertSerialNumber & { tenantId: string }): Promise<SerialNumber>;
  updateSerialNumber(tenantId: string, id: string, data: Partial<InsertSerialNumber>): Promise<SerialNumber>;
  deleteSerialNumber(tenantId: string, id: string): Promise<void>;

  // Inventory - Batch Numbers
  getBatchNumbers(tenantId: string, filters?: { itemId?: string; warehouseId?: string }): Promise<BatchNumber[]>;
  getBatchNumber(tenantId: string, id: string): Promise<BatchNumber | null>;
  createBatchNumber(data: InsertBatchNumber & { tenantId: string }): Promise<BatchNumber>;
  updateBatchNumber(tenantId: string, id: string, data: Partial<InsertBatchNumber>): Promise<BatchNumber>;
  deleteBatchNumber(tenantId: string, id: string): Promise<void>;

  // Inventory - Stock Adjustments
  getStockAdjustments(tenantId: string, filters?: { status?: string; warehouseId?: string }): Promise<StockAdjustment[]>;
  getStockAdjustment(tenantId: string, id: string): Promise<StockAdjustment | null>;
  createStockAdjustment(data: InsertStockAdjustment & { tenantId: string }, lineItems: InsertStockAdjustmentLineItem[]): Promise<StockAdjustment>;
  updateStockAdjustment(tenantId: string, id: string, data: Partial<InsertStockAdjustment>): Promise<StockAdjustment>;
  deleteStockAdjustment(tenantId: string, id: string): Promise<void>;
  updateStockAdjustmentStatus(tenantId: string, id: string, status: string): Promise<StockAdjustment>;
  getStockAdjustmentLineItems(tenantId: string, adjustmentId: string): Promise<StockAdjustmentLineItem[]>;

  // Inventory - Stock Counts
  getStockCounts(tenantId: string, filters?: { status?: string; warehouseId?: string }): Promise<StockCount[]>;
  getStockCount(tenantId: string, id: string): Promise<StockCount | null>;
  createStockCount(data: InsertStockCount & { tenantId: string }, lineItems: InsertStockCountLineItem[]): Promise<StockCount>;
  updateStockCount(tenantId: string, id: string, data: Partial<InsertStockCount>): Promise<StockCount>;
  deleteStockCount(tenantId: string, id: string): Promise<void>;
  updateStockCountStatus(tenantId: string, id: string, status: string): Promise<StockCount>;
  getStockCountLineItems(tenantId: string, countId: string): Promise<StockCountLineItem[]>;

  // Inventory - Composite Items
  getCompositeItemComponents(tenantId: string, compositeItemId: string): Promise<CompositeItemComponent[]>;
  createCompositeItemComponent(data: InsertCompositeItemComponent & { tenantId: string }): Promise<CompositeItemComponent>;
  updateCompositeItemComponent(tenantId: string, id: string, data: Partial<InsertCompositeItemComponent>): Promise<CompositeItemComponent>;
  deleteCompositeItemComponent(tenantId: string, id: string): Promise<void>;
  getActiveCompositeComponents(tenantId: string, compositeItemId: string): Promise<CompositeItemComponent[]>;

  // Inventory - UOM Conversions
  getUOMConversions(tenantId: string, itemId?: string): Promise<UOMConversion[]>;
  createUOMConversion(data: InsertUOMConversion & { tenantId: string }): Promise<UOMConversion>;
  updateUOMConversion(tenantId: string, id: string, data: Partial<InsertUOMConversion>): Promise<UOMConversion>;
  deleteUOMConversion(tenantId: string, id: string): Promise<void>;

  // Inventory - Cost Layers
  getInventoryCostLayers(tenantId: string, filters?: { itemId?: string; warehouseId?: string; isActive?: boolean }): Promise<InventoryCostLayer[]>;
  createInventoryCostLayer(data: InsertInventoryCostLayer & { tenantId: string }): Promise<InventoryCostLayer>;
  updateInventoryCostLayer(tenantId: string, id: string, data: Partial<InsertInventoryCostLayer>): Promise<InventoryCostLayer>;
  consumeCostLayer(tenantId: string, id: string, quantityToConsume: number): Promise<InventoryCostLayer>;

  // Inventory - Transactions
  getInventoryTransactions(tenantId: string, filters?: { itemId?: string; warehouseId?: string; transactionType?: string; startDate?: Date; endDate?: Date }): Promise<InventoryTransaction[]>;
  createInventoryTransaction(data: InsertInventoryTransaction & { tenantId: string }): Promise<InventoryTransaction>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async createWarehouse(data: InsertWarehouse & { tenantId: string }): Promise<Warehouse> {
    const [warehouse] = await db
      .insert(warehouses)
      .values(data)
      .returning();
  ...
  async updateWarehouse(tenantId: string, id: string, data: Partial<InsertWarehouse>): Promise<Warehouse> {
    const [updated] = await db
      .update(warehouses)
      .set({ ...data, updatedAt: new Date() })
      .where(and(
  ...
  async getDefaultWarehouse(tenantId: string): Promise<Warehouse | null> {
    const [warehouse] = await db
      .select()
      .from(warehouses)
      .where(and(
  ...
  async setDefaultWarehouse(tenantId: string, id: string): Promise<Warehouse> {
    return await db.transaction(async (tx) => {
      await tx
        .update(warehouses)
  ...
  async getWarehouseStock(tenantId: string, filters?: { warehouseId?: string; itemId?: string }): Promise<WarehouseStock[]> {
    let query = db
      .select()
      .from(warehouseStock)
    ...
    if (filters?.warehouseId) {
      query = query.where(and(
        ...
        eq(warehouseStock.warehouseId, filters.warehouseId)
      )) as any;
    }
    ...
    if (filters?.itemId) {
      query = query.where(and(
        eq(warehouseStock.tenantId, tenantId),
        eq(warehouseStock.itemId, filters.itemId)
      )) as any;
    }
  ...
  async getWarehouseStockByItem(tenantId: string, itemId: string): Promise<WarehouseStock[]> {
    return await db
      .select()
      .from(warehouseStock)
      .where(and(
  ...
  async getWarehouseStockByWarehouse(tenantId: string, warehouseId: string): Promise<WarehouseStock[]> {
    return await db
      .select()
      .from(warehouseStock)
      .where(and(
  ...
  async upsertWarehouseStock(data: InsertWarehouseStock & { tenantId: string }): Promise<WarehouseStock> {
    const [stock] = await db
      .insert(warehouseStock)
      .values(data)
      .onConflictDoUpdate({
        target: [warehouseStock.tenantId, warehouseStock.warehouseId, warehouseStock.itemId],
        set: {
          quantityOnHand: data.quantityOnHand,
          committedQuantity: data.committedQuantity || '0',
          availableQuantity: data.availableQuantity || data.quantityOnHand,
  ...
  async adjustWarehouseStock(tenantId: string, warehouseId: string, itemId: string, quantityChange: number): Promise<WarehouseStock> {
    ...
    const newQuantity = parseFloat(existing.quantityOnHand) + quantityChange;
    const newAvailable = parseFloat(existing.availableQuantity || existing.quantityOnHand) + quantityChange;
  ...
  // Inventory - Transfer Order operations
  async getTransferOrders(tenantId: string, filters?: { status?: string; fromWarehouseId?: string; toWarehouseId?: string }): Promise<TransferOrder[]> {
    let conditions = [eq(transferOrders.tenantId, tenantId)];
    
    if (filters?.status) {
      conditions.push(eq(transferOrders.status, filters.status));
    }
    if (filters?.fromWarehouseId) {
      conditions.push(eq(transferOrders.fromWarehouseId, filters.fromWarehouseId));
    }
    if (filters?.toWarehouseId) {
      conditions.push(eq(transferOrders.toWarehouseId, filters.toWarehouseId));
    }
  ...
  async getTransferOrder(tenantId: string, id: string): Promise<TransferOrder | null> {
  ...
  async createTransferOrder(data: InsertTransferOrder & { tenantId: string }, lineItems: InsertTransferOrderLineItem[]): Promise<TransferOrder> {
    return await db.transaction(async (tx) => {
      const [order] = await tx
      ...
      if (lineItems.length > 0) {
        ...
        await tx.insert(transferOrderLineItems).values(itemsWithOrderId);
      }
  ...
  async updateTransferOrder(tenantId: string, id: string, data: Partial<InsertTransferOrder>): Promise<TransferOrder> {
    const [updated] = await db
  ...
  async deleteTransferOrder(tenantId: string, id: string): Promise<void> {
    await db.transaction(async (tx) => {
      await tx
        .delete(transferOrderLineItems)
        .where(and(
          eq(transferOrderLineItems.tenantId, tenantId),
          eq(transferOrderLineItems.transferOrderId, id)
        ));
  ...
  async updateTransferOrderStatus(tenantId: string, id: string, status: string): Promise<TransferOrder> {
    const [updated] = await db
      .update(transferOrders)
  ...
  async getTransferOrderLineItems(tenantId: string, transferOrderId: string): Promise<TransferOrderLineItem[]> {
    return await db
      .select()
      .from(transferOrderLineItems)
      .where(and(
        eq(transferOrderLineItems.tenantId, tenantId),
        eq(transferOrderLineItems.transferOrderId, transferOrderId)
      ));
  }

  // Inventory - Serial Number operations
  async getSerialNumbers(tenantId: string, filters?: { itemId?: string; warehouseId?: string; status?: string }): Promise<SerialNumber[]> {
    let conditions = [eq(serialNumbers.tenantId, tenantId)];
    
    if (filters?.itemId) {
      conditions.push(eq(serialNumbers.itemId, filters.itemId));
    }
    if (filters?.warehouseId) {
  ...
  async getSerialNumber(tenantId: string, id: string): Promise<SerialNumber | null> {
  ...
  async createSerialNumber(data: InsertSerialNumber & { tenantId: string }): Promise<SerialNumber> {
    const [serial] = await db
  ...
  async updateSerialNumber(tenantId: string, id: string, data: Partial<InsertSerialNumber>): Promise<SerialNumber> {
    const [updated] = await db
  ...
  async deleteSerialNumber(tenantId: string, id: string): Promise<void> {
    await db
  ...
  // Inventory - Batch Number operations
  async getBatchNumbers(tenantId: string, filters?: { itemId?: string; warehouseId?: string }): Promise<BatchNumber[]> {
    let conditions = [eq(batchNumbers.tenantId, tenantId)];
    
    if (filters?.itemId) {
      conditions.push(eq(batchNumbers.itemId, filters.itemId));
    }
    if (filters?.warehouseId) {
      conditions.push(eq(batchNumbers.warehouseId, filters.warehouseId));
    }
  ...
  async getBatchNumber(tenantId: string, id: string): Promise<BatchNumber | null> {
    const [batch] = await db
      .select()
      .from(batchNumbers)
      .where(and(
        eq(batchNumbers.tenantId, tenantId),
        eq(batchNumbers.id, id)
      ));
    return batch || null;
  }

  async createBatchNumber(data: InsertBatchNumber & { tenantId: string }): Promise<BatchNumber> {
    const [batch] = await db
      .insert(batchNumbers)
      .values(data)
      .returning();
  ...
  async updateBatchNumber(tenantId: string, id: string, data: Partial<InsertBatchNumber>): Promise<BatchNumber> {
    const [updated] = await db
      .update(batchNumbers)
      .set({ ...data, updatedAt: new Date() })
      .where(and(
  ...
  async deleteBatchNumber(tenantId: string, id: string): Promise<void> {
    await db
      .delete(batchNumbers)
      .where(and(
        eq(batchNumbers.tenantId, tenantId),
  ...
  // Inventory - Stock Adjustment operations
  async getStockAdjustments(tenantId: string, filters?: { status?: string; warehouseId?: string }): Promise<StockAdjustment[]> {
    let conditions = [eq(stockAdjustments.tenantId, tenantId)];
    
    if (filters?.status) {
      conditions.push(eq(stockAdjustments.status, filters.status));
    }
    if (filters?.warehouseId) {
      conditions.push(eq(stockAdjustments.warehouseId, filters.warehouseId));
    }
  ...
  async getStockAdjustment(tenantId: string, id: string): Promise<StockAdjustment | null> {
  ...
  async createStockAdjustment(data: InsertStockAdjustment & { tenantId: string }, lineItems: InsertStockAdjustmentLineItem[]): Promise<StockAdjustment> {
    return await db.transaction(async (tx) => {
      ...
      if (lineItems.length > 0) {
        const itemsWithAdjustmentId = lineItems.map(item => ({
          ...item,
          adjustmentId: adjustment.id,
          tenantId: data.tenantId,
        }));
        await tx.insert(stockAdjustmentLineItems).values(itemsWithAdjustmentId);
      }
  ...
  async updateStockAdjustment(tenantId: string, id: string, data: Partial<InsertStockAdjustment>): Promise<StockAdjustment> {
    const [updated] = await db
  ...
  async deleteStockAdjustment(tenantId: string, id: string): Promise<void> {
    await db.transaction(async (tx) => {
      await tx
        .delete(stockAdjustmentLineItems)
        .where(and(
          eq(stockAdjustmentLineItems.tenantId, tenantId),
          eq(stockAdjustmentLineItems.adjustmentId, id)
        ));
  ...
  async updateStockAdjustmentStatus(tenantId: string, id: string, status: string): Promise<StockAdjustment> {
    const [updated] = await db
      .update(stockAdjustments)
      .set({ status, updatedAt: new Date() })
      .where(and(
  ...
  async getStockAdjustmentLineItems(tenantId: string, adjustmentId: string): Promise<StockAdjustmentLineItem[]> {
    return await db
      .select()
      .from(stockAdjustmentLineItems)
      .where(and(
        eq(stockAdjustmentLineItems.tenantId, tenantId),
        eq(stockAdjustmentLineItems.adjustmentId, adjustmentId)
      ));
  }

  // Inventory - Stock Count operations
  async getStockCounts(tenantId: string, filters?: { status?: string; warehouseId?: string }): Promise<StockCount[]> {
    let conditions = [eq(stockCounts.tenantId, tenantId)];
    
    if (filters?.status) {
      conditions.push(eq(stockCounts.status, filters.status));
    }
    if (filters?.warehouseId) {
      conditions.push(eq(stockCounts.warehouseId, filters.warehouseId));
    }
  ...
  async getStockCount(tenantId: string, id: string): Promise<StockCount | null> {
  ...
  async createStockCount(data: InsertStockCount & { tenantId: string }, lineItems: InsertStockCountLineItem[]): Promise<StockCount> {
    return await db.transaction(async (tx) => {
      ...
      if (lineItems.length > 0) {
        const itemsWithCountId = lineItems.map(item => ({
          ...item,
          countId: count.id,
          tenantId: data.tenantId,
        }));
        await tx.insert(stockCountLineItems).values(itemsWithCountId);
      }
  ...
  async updateStockCount(tenantId: string, id: string, data: Partial<InsertStockCount>): Promise<StockCount> {
    const [updated] = await db
      .update(stockCounts)
      .set({ ...data, updatedAt: new Date() })
      .where(and(
  ...
  async deleteStockCount(tenantId: string, id: string): Promise<void> {
    await db.transaction(async (tx) => {
      await tx
        .delete(stockCountLineItems)
        .where(and(
          eq(stockCountLineItems.tenantId, tenantId),
          eq(stockCountLineItems.countId, id)
        ));
  ...
  async updateStockCountStatus(tenantId: string, id: string, status: string): Promise<StockCount> {
    const [updated] = await db
      .update(stockCounts)
      .set({ status, updatedAt: new Date() })
      .where(and(
  ...
  async getStockCountLineItems(tenantId: string, countId: string): Promise<StockCountLineItem[]> {
    return await db
      .select()
      .from(stockCountLineItems)
      .where(and(
        eq(stockCountLineItems.tenantId, tenantId),
        eq(stockCountLineItems.countId, countId)
      ));
  }

  // Inventory - Composite Item Component operations
  async getCompositeItemComponents(tenantId: string, compositeItemId: string): Promise<CompositeItemComponent[]> {
    return await db
      .select()
      .from(compositeItemComponents)
      .where(and(
        eq(compositeItemComponents.tenantId, tenantId),
        eq(compositeItemComponents.compositeItemId, compositeItemId)
      ))
      .orderBy(desc(compositeItemComponents.createdAt));
  }

  async createCompositeItemComponent(data: InsertCompositeItemComponent & { tenantId: string }): Promise<CompositeItemComponent> {
    const [component] = await db
      .insert(compositeItemComponents)
      .values(data)
      .returning();
    return component;
  }

  async updateCompositeItemComponent(tenantId: string, id: string, data: Partial<InsertCompositeItemComponent>): Promise<CompositeItemComponent> {
    const [updated] = await db
      .update(compositeItemComponents)
      .set({ ...data, updatedAt: new Date() })
      .where(and(
        eq(compositeItemComponents.tenantId, tenantId),
        eq(compositeItemComponents.id, id)
      ))
      .returning();
    
    if (!updated) {
      throw new Error('Composite item component not found');
    }
    
    return updated;
  }

  async deleteCompositeItemComponent(tenantId: string, id: string): Promise<void> {
    await db
      .delete(compositeItemComponents)
      .where(and(
        eq(compositeItemComponents.tenantId, tenantId),
        eq(compositeItemComponents.id, id)
      ));
  ...
  async getActiveCompositeComponents(tenantId: string, compositeItemId: string): Promise<CompositeItemComponent[]> {
    const now = new Date();
    return await db
      .select()
      .from(compositeItemComponents)
      .where(and(
        eq(compositeItemComponents.tenantId, tenantId),
        eq(compositeItemComponents.compositeItemId, compositeItemId),
        eq(compositeItemComponents.isActive, true),
        sql`(${compositeItemComponents.effectiveTo} IS NULL OR ${compositeItemComponents.effectiveTo} > ${now})`
      ))
      .orderBy(desc(compositeItemComponents.createdAt));
  }

  // Inventory - UOM Conversion operations
  async getUOMConversions(tenantId: string, itemId?: string): Promise<UOMConversion[]> {
    if (itemId) {
      return await db
        .select()
        .from(uomConversions)
        .where(and(
          ...
          eq(uomConversions.itemId, itemId)
        ))
        .orderBy(desc(uomConversions.createdAt));
    }
    
    return await db
      .select()
      .from(uomConversions)
      .where(eq(uomConversions.tenantId, tenantId))
      .orderBy(desc(uomConversions.createdAt));
  }

  async createUOMConversion(data: InsertUOMConversion & { tenantId: string }): Promise<UOMConversion> {
  ...
  async updateUOMConversion(tenantId: string, id: string, data: Partial<InsertUOMConversion>): Promise<UOMConversion> {
    const [updated] = await db
      .update(uomConversions)
      .set({ ...data, updatedAt: new Date() })
      .where(and(
        eq(uomConversions.tenantId, tenantId),
        eq(uomConversions.id, id)
      ))
      .returning();
  ...
  async deleteUOMConversion(tenantId: string, id: string): Promise<void> {
    await db
  ...
  // Inventory - Cost Layer operations
  async getInventoryCostLayers(tenantId: string, filters?: { itemId?: string; warehouseId?: string; isActive?: boolean }): Promise<InventoryCostLayer[]> {
    let conditions = [eq(inventoryCostLayers.tenantId, tenantId)];
    
    if (filters?.itemId) {
      conditions.push(eq(inventoryCostLayers.itemId, filters.itemId));
    }
    if (filters?.warehouseId) {
      conditions.push(eq(inventoryCostLayers.warehouseId, filters.warehouseId));
    }
    if (filters?.isActive !== undefined) {
      conditions.push(eq(inventoryCostLayers.isActive, filters.isActive));
    }
    ...
    return await db
      ...
      .where(and(...conditions))
      .orderBy(asc(inventoryCostLayers.acquisitionDate));
  }

  async createInventoryCostLayer(data: InsertInventoryCostLayer & { tenantId: string }): Promise<InventoryCostLayer> {
  ...
  async updateInventoryCostLayer(tenantId: string, id: string, data: Partial<InsertInventoryCostLayer>): Promise<InventoryCostLayer> {
    const [updated] = await db
    ...
    if (!updated) {
      throw new Error('Inventory cost layer not found');
    }
    
    return updated;
  }

  async consumeCostLayer(tenantId: string, id: string, quantityToConsume: number): Promise<InventoryCostLayer> {
    const [existing] = await db
      .select()
      .from(inventoryCostLayers)
      .where(and(
    ...
    const currentRemaining = parseFloat(existing.remainingQuantity);
    const newRemaining = currentRemaining - quantityToConsume;
  ...
  async getInventoryTransactions(tenantId: string, filters?: { itemId?: string; warehouseId?: string; transactionType?: string; startDate?: Date; endDate?: Date }): Promise<InventoryTransaction[]> {
    ...
    if (filters?.startDate) {
      conditions.push(gte(inventoryTransactions.transactionDate, filters.startDate));
    }
    if (filters?.endDate) {
      conditions.push(lte(inventoryTransactions.transactionDate, filters.endDate));
    }
  ...
  async createInventoryTransaction(data: InsertInventoryTransaction & { tenantId: string }): Promise<InventoryTransaction> {
