...
import { fetchInvoiceEntryData, createInvoiceJournalEntry } from './accounting/entry-creators';
  ...
  app.post("/api/invoices/:id/post", isAuthenticated, verifyTenantAccess, loadAuthContext, requirePermission('invoices.update'), async (req: any, res) => {
    ...
    try {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        // 3c. Get next journal entry number
        const journalEntryNumber = await storage.getNextJournalEntryNumber(tenantId);

        // 3d. Persist journal entry header
        ...
        // 3e. Persist journal entry lines (legs)
