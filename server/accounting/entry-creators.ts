...
/**
 * Stock Adjustment Line Item Entry Input
 * 
 * Normalized structure for stock adjustment line items used in journal entry creation.
 */
export interface StockAdjustmentLineEntryInput {
  itemId: string;
  quantityAdjusted: string; // Can be positive or negative
  unitCost: string;
  totalCost: string;        // quantityAdjusted * unitCost
}

/**
 * Stock Adjustment Entry Input
 * 
 * Normalized structure for stock adjustments used in journal entry creation.
 * 
 * **Journal Entry Pattern:**
 * ```
 * If adjustment > 0 (gain):
 *   DR  Inventory Asset       [totalCost]
 *       CR  Inventory Adjustment [totalCost]
 * 
 * If adjustment < 0 (loss):
 *   DR  Inventory Adjustment  [totalCost]
 *       CR  Inventory Asset   [totalCost]
 * ```
 */
export interface StockAdjustmentEntryInput {
  id: string;
  adjustmentNumber: string;
  adjustmentDate: Date;
  reason: string;
  lineItems: StockAdjustmentLineEntryInput[];
}

/**
 * Opening Stock Entry Input
 * 
 * Normalized structure for opening stock used in journal entry creation.
 * 
 * **Journal Entry Pattern:**
 * ```
 * DR  Inventory Asset         [totalValue]
 *     CR  Opening Balance Equity [totalValue]
 * ```
 */
export interface OpeningStockEntryInput {
  ...
  openingRate: string;
  totalValue: string;      // openingQuantity * openingRate
}

/**
 * Expense Approval Entry Input
 * 
 * Normalized structure for expense approvals used in journal entry creation.
 * 
 * **Journal Entry Pattern:**
 * ```
 * DR  Expense Account         [totalAmount]
 *     CR  Employee Reimbursement Payable [totalAmount]
 * ```
 */
export interface ExpenseApprovalEntryInput {
  id: string;
  expenseNumber: string;
  expenseDate: Date;
  employeeName: string;
  totalAmount: string;
  categoryAccountId: string; // Expense account to debit
}

/**
 * Expense Reimbursement Entry Input
 * 
 * Normalized structure for expense reimbursements used in journal entry creation.
 * 
 * **Journal Entry Pattern:**
 * ```
 * DR  Employee Reimbursement Payable [amount]
 *     CR  Cash/Bank           [amount]
 * ```
 */
export interface ExpenseReimbursementEntryInput {
  expenseId: string;
  expenseNumber: string;
...
/**
 * Create journal entry for a stock adjustment
 * 
 * **Journal Entry Pattern:**
 * ```
 * If adjustment > 0 (gain):
 ...
 * ```
 * 
 * @param input - Normalized stock adjustment entry input
 * @param tenantId - Tenant ID for security isolation
 * @param userId - User ID who is creating this entry (for preparedBy tracking)
 * @param storage - Storage instance for database access
 * @param tx - Database transaction for atomicity
...
export async function createStockAdjustmentJournalEntry(
  input: StockAdjustmentEntryInput,
  tenantId: string,
  userId: string,
  storage: IStorage,
  tx: DBTransaction
  ...
  // Aggregate total adjustment value
  let totalAdjustmentValue = 0;
  for (const lineItem of input.lineItems) {
    totalAdjustmentValue += parseFloat(lineItem.totalCost);
  }
  
  if (totalAdjustmentValue > 0) {
    // Gain: Inventory increased
    // DR Inventory Asset
    lines.push({
      accountId: sysAccounts.inventory.id,
      debitAmount: totalAdjustmentValue.toFixed(2),
      description: `Stock adjustment ${input.adjustmentNumber} - ${input.reason}`,
    });
    
    // CR Inventory Adjustment
    lines.push({
      accountId: sysAccounts.inventoryAdjustment.id,
      creditAmount: totalAdjustmentValue.toFixed(2),
      description: `Stock adjustment ${input.adjustmentNumber} - Inventory gain`,
    });
  } else if (totalAdjustmentValue < 0) {
    // Loss: Inventory decreased
    const absValue = Math.abs(totalAdjustmentValue).toFixed(2);
    
    // DR Inventory Adjustment
    lines.push({
      accountId: sysAccounts.inventoryAdjustment.id,
      debitAmount: absValue,
      description: `Stock adjustment ${input.adjustmentNumber} - Inventory loss`,
    });
  ...
  return {
    ...
    referenceNumber: input.adjustmentNumber,
    sourceDocumentType: 'expense', // Using expense as closest match, could add 'stock_adjustment' to enum
    sourceDocumentId: input.id,
    isAutoGenerated: true,
    preparedBy: userId,
    preparedAt: new Date(),
 ...
 * ```
 * 
 * @param input - Normalized opening stock entry input
 * @param tenantId - Tenant ID for security isolation
 * @param userId - User ID who is creating this entry (for preparedBy tracking)
 * @param storage - Storage instance for database access
 * @param tx - Database transaction for atomicity
 * @returns Created journal entry
 */
export async function createOpeningStockJournalEntry(
  input: OpeningStockEntryInput,
  tenantId: string,
  userId: string,
  storage: IStorage,
  tx: DBTransaction
  ...
  lines.push({
    accountId: sysAccounts.inventory.id,
    debitAmount: input.totalValue,
    description: `Opening stock - ${input.itemName}`,
  });
  
  // CR Opening Balance Equity
  lines.push({
    accountId: sysAccounts.openingBalanceEquity.id,
    creditAmount: input.totalValue,
    description: `Opening stock - ${input.itemName}`,
  });
  ...
  return {
    entryDate: new Date(), // Use current date for opening stock
    description: `Opening Stock - ${input.itemName}`,
    referenceNumber: `OPEN-${input.itemId}`,
    sourceDocumentType: 'expense', // Using expense as closest match
    sourceDocumentId: input.itemId,
    isAutoGenerated: true,
    preparedBy: userId,
    preparedAt: new Date(),
    lines,
 ...
 * ```
 * 
 * @param input - Normalized expense approval entry input
 * @param tenantId - Tenant ID for security isolation
 * @param userId - User ID who is creating this entry (for preparedBy tracking)
 * @param storage - Storage instance for database access
 * @param tx - Database transaction for atomicity
...
export async function createExpenseApprovalJournalEntry(
  input: ExpenseApprovalEntryInput,
  tenantId: string,
  userId: string,
  storage: IStorage,
  tx: DBTransaction
  ...
  lines.push({
    ...
    debitAmount: input.totalAmount,
    description: `Employee expense ${input.expenseNumber} - ${input.employeeName}`,
  });
  
  // CR Employee Reimbursement Payable
  lines.push({
    accountId: sysAccounts.employeeReimbursementPayable.id,
    creditAmount: input.totalAmount,
    description: `Employee expense ${input.expenseNumber} - Reimbursement due to ${input.employeeName}`,
  });
  ...
  return {
    ...
    description: `Employee Expense ${input.expenseNumber}`,
    referenceNumber: input.expenseNumber,
 ...
 * ```
 * 
 * @param input - Normalized expense reimbursement entry input
 * @param tenantId - Tenant ID for security isolation
 * @param userId - User ID who is creating this entry (for preparedBy tracking)
 * @param storage - Storage instance for database access
 * @param tx - Database transaction for atomicity
 * @returns Created journal entry
 */
export async function createExpenseReimbursementJournalEntry(
  input: ExpenseReimbursementEntryInput,
  tenantId: string,
  userId: string,
  storage: IStorage,
  tx: DBTransaction
  ...
  lines.push({
    accountId: sysAccounts.employeeReimbursementPayable.id,
    debitAmount: input.amount,
    description: `Expense reimbursement ${input.expenseNumber} - Payment to employee`,
  });
  ...
  lines.push({
    accountId: sysAccounts.cash.id,
    creditAmount: input.amount,
    description: `Expense reimbursement ${input.expenseNumber} - ${input.paymentMethod}`,
  });
  ...
  return {
    entryDate: input.reimbursementDate,
    description: `Expense Reimbursement ${input.expenseNumber}`,
    referenceNumber: input.expenseNumber,
    sourceDocumentType: 'expense',
    sourceDocumentId: input.expenseId,
    isAutoGenerated: true,
    preparedBy: userId,
    preparedAt: new Date(),
    lines,
