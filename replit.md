...
- **Automatic Journal Entries (Phase 3 - COMPLETED CORE):** Complete double-entry bookkeeping automation with all posting endpoints operational:
    - **Journal Entry Creators:** Implemented for all 6 major document types (Invoice, Bill, Customer Payment, Vendor Payment, Credit Note, Debit Note) with proper double-entry patterns
    ...
    - **Transaction-Aware Storage Layer:** All journal entry creation methods (getNextJournalEntryNumber, createJournalEntry, createJournalEntryLegs) honor provided transactions for true atomicity
    - **Posting Endpoints (ALL OPERATIONAL):**
        - POST /api/invoices/:id/post - Creates journal entries (DR AR, CR Revenue, CR Tax) with COGS entries for inventory items
        - POST /api/bills/:id/post - Creates journal entries (DR Expense per line, DR Input Tax, CR AP)
        - POST /api/customer-payments - Integrated journal entry creation (DR Cash/Bank, CR AR)
        - POST /api/payments - Integrated journal entry creation for vendor payments (DR AP, CR Cash/Bank)
        - POST /api/credit-notes/:id/post - Creates journal entries (DR Revenue, DR Tax Payable, CR AR)
        - POST /api/debit-notes/:id/post - Creates journal entries (DR AP, CR Expense, CR Tax Receivable)
    - **Atomic Transaction Pattern:** All posting endpoints use withTransaction wrapper ensuring document status updates and journal entry creation commit/rollback together
    - **RBAC Integration:** All posting endpoints protected with appropriate permissions (bills.approve_posting, credit_notes.approve, debit_notes.approve, customer_payments.create, payments.create)
    - **Audit Metadata:** All auto-generated entries track preparer (preparedBy, preparedAt), generation source (isAutoGenerated=true, sourceDocumentType, sourceDocumentId), and are locked (modificationLocked=true)
    - **Status Validation:** Prevents double-posting with proper status checks (e.g., only unpaid bills can be posted)
    - **Next Steps:** Implement approval workflows with graphical builder, complete historical balance service, build workflow UI components
