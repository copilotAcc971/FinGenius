...
// Inventory Management Status Enums
export const TRANSFER_ORDER_STATUS = ['draft', 'pending', 'in_transit', 'received', 'cancelled'] as const;
export const STOCK_ADJUSTMENT_STATUS = ['draft', 'pending_approval', 'approved', 'rejected'] as const;
export const STOCK_COUNT_STATUS = ['draft', 'in_progress', 'completed', 'cancelled'] as const;
export const INVENTORY_TRANSACTION_TYPE = ['purchase', 'sale', 'adjustment', 'transfer_out', 'transfer_in', 'count', 'return'] as const;
export const INVENTORY_VALUATION_METHOD = ['fifo', 'lifo', 'weighted_average', 'standard_cost'] as const;
...
export const items = pgTable("items", {
  ...
  openingStock: decimal("opening_stock", { precision: 10, scale: 2 }).default("0"), // Opening balance
  openingStockDate: timestamp("opening_stock_date"),
  openingStockRate: decimal("opening_stock_rate", { precision: 12, scale: 2 }), // Opening stock value per unit
  
  // Stock availability breakdown
  availableForSale: decimal("available_for_sale", { precision: 10, scale: 2 }).default("0"), // Calculated: quantityOnHand - committed
  committed: decimal("committed", { precision: 10, scale: 2 }).default("0"), // Reserved for sales orders
  inTransit: decimal("in_transit", { precision: 10, scale: 2 }).default("0"), // From transfer orders
  yetToReceive: decimal("yet_to_receive", { precision: 10, scale: 2 }).default("0"), // From open purchase orders
  
  // Reorder management
  reorderLevel: decimal("reorder_level", { precision: 10, scale: 2 }),
  reorderQuantity: decimal("reorder_quantity", { precision: 10, scale: 2 }),
  preferredVendorId: varchar("preferred_vendor_id").references(() => vendors.id),
  
  // Inventory valuation
  valuationMethod: varchar("valuation_method", { length: 50 }).default('weighted_average'), // fifo, lifo, weighted_average, standard_cost
  averageCost: decimal("average_cost", { precision: 12, scale: 2 }).default("0"), // Weighted average cost
  standardCost: decimal("standard_cost", { precision: 12, scale: 2 }), // For standard costing method
  
  // Serial/Batch tracking
  trackSerialNumbers: boolean("track_serial_numbers").default(false).notNull(),
  trackBatchNumbers: boolean("track_batch_numbers").default(false).notNull(),
  
  // Composite items (kits/bundles)
  isComposite: boolean("is_composite").default(false).notNull(),
  compositeCost: decimal("composite_cost", { precision: 12, scale: 2 }), // Total cost of components
  
  // Unit of Measure (UOM)
  defaultUOM: varchar("default_uom", { length: 50 }), // Primary unit (e.g., "pcs", "kg", "liters")
  
  // Item metadata and images
  itemCategory: varchar("item_category", { length: 100 }),
  manufacturer: varchar("manufacturer", { length: 255 }),
  brand: varchar("brand", { length: 255 }),
  itemImages: text("item_images").array(), // Array of image URLs
  ...
  unique("items_tenant_sku").on(table.tenantId, table.sku),
]);
...
// Warehouse Stock Levels (stock by warehouse - per-warehouse quantity tracking)
export const warehouseStock = pgTable("warehouse_stock", {
  ...
  quantityOnHand: decimal("quantity_on_hand", { precision: 10, scale: 2 }).default("0").notNull(),
  committed: decimal("committed", { precision: 10, scale: 2 }).default("0").notNull(), // Reserved for sales orders
  inTransit: decimal("in_transit", { precision: 10, scale: 2 }).default("0").notNull(), // From transfer orders
  availableForSale: decimal("available_for_sale", { precision: 10, scale: 2 }).default("0").notNull(), // quantityOnHand - committed
  
  // Reorder management per warehouse
  reorderLevel: decimal("reorder_level", { precision: 10, scale: 2 }),
  reorderQuantity: decimal("reorder_quantity", { precision: 10, scale: 2 }),
  ...
  index("warehouse_stock_tenant_idx").on(table.tenantId),
  index("warehouse_stock_warehouse_idx").on(table.warehouseId),
  index("warehouse_stock_item_idx").on(table.itemId),
  unique("warehouse_stock_unique").on(table.tenantId, table.warehouseId, table.itemId),
]);
...
export const transferOrders = pgTable("transfer_orders", {
  ...
  status: varchar("status", { length: 50 }).notNull().default("draft"), // draft, pending, in_transit, received, cancelled
  notes: text("notes"),
  ...
  index("transfer_orders_from_warehouse_idx").on(table.fromWarehouseId),
  index("transfer_orders_to_warehouse_idx").on(table.toWarehouseId),
  unique("transfer_orders_number").on(table.tenantId, table.transferNumber),
]);
...
export const transferOrderLineItems = pgTable("transfer_order_line_items", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").notNull().references(() => tenants.id),
  transferOrderId: varchar("transfer_order_id").notNull().references(() => transferOrders.id, { onDelete: 'cascade' }),
  itemId: varchar("item_id").notNull().references(() => items.id),
  quantityShipped: decimal("quantity_shipped", { precision: 10, scale: 2 }).notNull(),
  quantityReceived: decimal("quantity_received", { precision: 10, scale: 2 }).default("0").notNull(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => [
  index("transfer_order_line_items_tenant_idx").on(table.tenantId),
  index("transfer_order_line_items_transfer_idx").on(table.transferOrderId),
  index("transfer_order_line_items_item_idx").on(table.itemId),
]);

export const insertTransferOrderLineItemSchema = createInsertSchema(transferOrderLineItems, {
  quantityShipped: decimalString,
...
export type InsertTransferOrderLineItem = z.infer<typeof insertTransferOrderLineItemSchema>;
export type TransferOrderLineItem = typeof transferOrderLineItems.$inferSelect;
...
export const serialNumbers = pgTable("serial_numbers", {
  ...
  soldToCustomerId: varchar("sold_to_customer_id").references(() => customers.id),
  notes: text("notes"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => [
  index("serial_numbers_tenant_idx").on(table.tenantId),
  index("serial_numbers_item_idx").on(table.itemId),
  index("serial_numbers_warehouse_idx").on(table.warehouseId),
  unique("serial_numbers_unique").on(table.tenantId, table.itemId, table.serialNumber),
]);
...
// Batch Numbers
export const batchNumbers = pgTable("batch_numbers", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").notNull().references(() => tenants.id),
  itemId: varchar("item_id").notNull().references(() => items.id, { onDelete: 'cascade' }),
  ...
  index("batch_numbers_tenant_idx").on(table.tenantId),
  index("batch_numbers_item_idx").on(table.itemId),
  index("batch_numbers_warehouse_idx").on(table.warehouseId),
  index("batch_numbers_expiry_idx").on(table.expiryDate),
  unique("batch_numbers_unique").on(table.tenantId, table.itemId, table.batchNumber),
]);

export const insertBatchNumberSchema = createInsertSchema(batchNumbers, {
...
export type InsertBatchNumber = z.infer<typeof insertBatchNumberSchema>;
export type BatchNumber = typeof batchNumbers.$inferSelect;
...
export const stockAdjustments = pgTable("stock_adjustments", {
  ...
  reason: varchar("reason", { length: 255 }).notNull(), // damage, theft, shrinkage, correction, found, etc.
  status: varchar("status", { length: 50 }).notNull().default("draft"), // draft, pending_approval, approved, rejected
  notes: text("notes"),
  createdBy: varchar("created_by").references(() => users.id),
  ...
  index("stock_adjustments_tenant_idx").on(table.tenantId),
  index("stock_adjustments_warehouse_idx").on(table.warehouseId),
  unique("stock_adjustments_number").on(table.tenantId, table.adjustmentNumber),
]);
...
export const stockAdjustmentLineItems = pgTable("stock_adjustment_line_items", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").notNull().references(() => tenants.id),
  stockAdjustmentId: varchar("stock_adjustment_id").notNull().references(() => stockAdjustments.id, { onDelete: 'cascade' }),
  itemId: varchar("item_id").notNull().references(() => items.id),
  ...
  quantityAdjusted: decimal("quantity_adjusted", { precision: 10, scale: 2 }).notNull(), // Can be negative
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => [
  index("stock_adjustment_line_items_tenant_idx").on(table.tenantId),
  index("stock_adjustment_line_items_adjustment_idx").on(table.stockAdjustmentId),
  index("stock_adjustment_line_items_item_idx").on(table.itemId),
]);

export const insertStockAdjustmentLineItemSchema = createInsertSchema(stockAdjustmentLineItems, {
  quantityBefore: decimalString,
  quantityAfter: decimalString,
  quantityAdjusted: decimalString,
}).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
...
export type InsertStockAdjustmentLineItem = z.infer<typeof insertStockAdjustmentLineItemSchema>;
...
export const stockCounts = pgTable("stock_counts", {
  ...
  index("stock_counts_tenant_idx").on(table.tenantId),
  index("stock_counts_warehouse_idx").on(table.warehouseId),
  unique("stock_counts_number").on(table.tenantId, table.countNumber),
]);
...
// Stock Count Line Items
export const stockCountLineItems = pgTable("stock_count_line_items", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").notNull().references(() => tenants.id),
  stockCountId: varchar("stock_count_id").notNull().references(() => stockCounts.id, { onDelete: 'cascade' }),
  itemId: varchar("item_id").notNull().references(() => items.id),
  ...
  variance: decimal("variance", { precision: 10, scale: 2 }), // actualQuantity - expectedQuantity
  notes: text("notes"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => [
  index("stock_count_line_items_tenant_idx").on(table.tenantId),
  index("stock_count_line_items_count_idx").on(table.stockCountId),
  index("stock_count_line_items_item_idx").on(table.itemId),
]);

export const insertStockCountLineItemSchema = createInsertSchema(stockCountLineItems, {
...
export type InsertStockCountLineItem = z.infer<typeof insertStockCountLineItemSchema>;
export type StockCountLineItem = typeof stockCountLineItems.$inferSelect;

// Composite Item Components (for kits/bundles with BOM - Bill of Materials)
export const compositeItemComponents = pgTable("composite_item_components", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").notNull().references(() => tenants.id),
  compositeItemId: varchar("composite_item_id").notNull().references(() => items.id, { onDelete: 'cascade' }),
  componentItemId: varchar("component_item_id").notNull().references(() => items.id, { onDelete: 'cascade' }),
  
  // Component quantity and cost
  quantity: decimal("quantity", { precision: 10, scale: 2 }).notNull(),
  componentCost: decimal("component_cost", { precision: 12, scale: 2 }), // Cost per unit of component
  totalComponentCost: decimal("total_component_cost", { precision: 12, scale: 2 }), // quantity * componentCost
  
  // Effective dating for BOM versions
  effectiveFrom: timestamp("effective_from").defaultNow().notNull(),
  effectiveTo: timestamp("effective_to"), // NULL means currently active
  
  // Sorting order for assembly instructions
  ...
  index("composite_item_components_tenant_idx").on(table.tenantId),
  index("composite_item_components_composite_idx").on(table.compositeItemId),
  index("composite_item_components_component_idx").on(table.componentItemId),
  index("composite_item_components_effective_idx").on(table.effectiveFrom, table.effectiveTo),
  unique("composite_item_components_unique").on(table.tenantId, table.compositeItemId, table.componentItemId, table.effectiveFrom),
]);

export const insertCompositeItemComponentSchema = createInsertSchema(compositeItemComponents, {
  quantity: decimalString,
  componentCost: decimalString,
...
export type InsertCompositeItemComponent = z.infer<typeof insertCompositeItemComponentSchema>;
export type CompositeItemComponent = typeof compositeItemComponents.$inferSelect;
...
export const uomConversions = pgTable("uom_conversions", {
  ...
  conversionFactor: decimal("conversion_factor", { precision: 10, scale: 6 }).notNull(), // e.g., 1 box = 12 pieces, factor = 12
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => [
  index("uom_conversions_tenant_idx").on(table.tenantId),
  index("uom_conversions_item_idx").on(table.itemId),
  unique("uom_conversions_unique").on(table.tenantId, table.itemId, table.fromUOM, table.toUOM),
]);

export const insertUOMConversionSchema = createInsertSchema(uomConversions, {
...
// Inventory Cost Layers (for FIFO/LIFO/Weighted Average valuation)
export const inventoryCostLayers = pgTable("inventory_cost_layers", {
  ...
  // Cost layer details
  receiptDate: timestamp("receipt_date").notNull(), // When this cost layer was created
  quantity: decimal("quantity", { precision: 10, scale: 2 }).notNull(), // Original quantity in this layer
  remainingQuantity: decimal("remaining_quantity", { precision: 10, scale: 2 }).notNull(), // Quantity still available (for FIFO/LIFO)
  unitCost: decimal("unit_cost", { precision: 12, scale: 2 }).notNull(), // Cost per unit
  totalCost: decimal("total_cost", { precision: 15, scale: 2 }).notNull(), // Total cost of this layer
  
  // Source document
  sourceType: varchar("source_type", { length: 50 }).notNull(), // bill, transfer_in, stock_adjustment, opening_stock
  sourceId: varchar("source_id"),
  sourceNumber: varchar("source_number", { length: 100 }),
  
  // Serial/Batch tracking
  serialNumberId: varchar("serial_number_id").references(() => serialNumbers.id),
  batchNumberId: varchar("batch_number_id").references(() => batchNumbers.id),
  
  isActive: boolean("is_active").default(true).notNull(), // False when fully consumed
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => [
  index("inventory_cost_layers_tenant_idx").on(table.tenantId),
  index("inventory_cost_layers_item_idx").on(table.itemId),
  index("inventory_cost_layers_warehouse_idx").on(table.warehouseId),
  index("inventory_cost_layers_receipt_date_idx").on(table.receiptDate),
  index("inventory_cost_layers_active_idx").on(table.isActive),
]);
...
// Inventory Transactions Ledger (audit trail of all stock movements)
export const inventoryTransactions = pgTable("inventory_transactions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").notNull().references(() => tenants.id),
  itemId: varchar("item_id").notNull().references(() => items.id),
  warehouseId: varchar("warehouse_id").references(() => warehouses.id),
  transactionType: varchar("transaction_type", { length: 50 }).notNull(), // purchase, sale, adjustment, transfer_out, transfer_in, count, return
  transactionDate: timestamp("transaction_date").notNull(),
  quantity: decimal("quantity", { precision: 10, scale: 2 }).notNull(), // Positive for increases, negative for decreases
  quantityBefore: decimal("quantity_before", { precision: 10, scale: 2 }).notNull(),
  quantityAfter: decimal("quantity_after", { precision: 10, scale: 2 }).notNull(),
  unitCost: decimal("unit_cost", { precision: 12, scale: 2 }),
  totalValue: decimal("total_value", { precision: 15, scale: 2 }),
  
  // Link to cost layer for FIFO/LIFO/WAC valuation
  costLayerId: varchar("cost_layer_id").references(() => inventoryCostLayers.id),
  
  // Reference to source document
  referenceType: varchar("reference_type", { length: 50 }), // invoice, bill, transfer_order, stock_adjustment, stock_count
  ...
  index("inventory_transactions_tenant_idx").on(table.tenantId),
  index("inventory_transactions_item_idx").on(table.itemId),
  index("inventory_transactions_warehouse_idx").on(table.warehouseId),
  index("inventory_transactions_date_idx").on(table.transactionDate),
  index("inventory_transactions_type_idx").on(table.transactionType),
