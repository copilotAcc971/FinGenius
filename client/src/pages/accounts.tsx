    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage your accounting accounts and categories</p>
        </div>
        <Button
          onClick={() => {
            ...
            <TableBody>
              {filteredAccounts.map((account) => (
                <TableRow key={account.id} data-testid={`row-account-${account.id}`}>
                  <TableCell className="font-mono font-medium" data-testid={`text-code-${account.id}`}>
                    {account.code}
                  </TableCell>
                  <TableCell className="font-medium" data-testid={`text-name-${account.id}`}>
                    {account.name}
                  </TableCell>
                  <TableCell>
                    <Badge variant={getAccountTypeBadgeVariant(account.type)} data-testid={`badge-type-${account.id}`}>
                      {account.type.charAt(0).toUpperCase() + account.type.slice(1)}
                    </Badge>
                  </TableCell>
                  <TableCell data-testid={`text-category-${account.id}`}>
                    {account.accountCategory || "-"}
                  </TableCell>
                  <TableCell className="text-right font-mono" data-testid={`text-balance-${account.id}`}>
