  ...
  return useMutation({
    ...
    onSuccess: (data, variables, context) => {
      if (data && context?.optimisticId) {
        // CRITICAL: Replace optimistic item with server data, explicitly clearing isPending
      ...
      } else if (!data && context?.optimisticId) {
        // CRITICAL: Handle 204/empty responses - remove optimistic item and refetch
        queryClient.setQueryData<TData>(queryKey, (old) => {
          if (!old) return [] as TData;
          return old.filter((item: any) => String(item.id) !== context.optimisticId) as TData;
        });
        // Trigger refetch to get the actual created item
